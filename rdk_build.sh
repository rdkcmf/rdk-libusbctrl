...
if [ "x${RDK_PLATFORM_SOC^^}" == "xBROADCOM" ];then
...
if [ "x${RDK_PLATFORM_SOC^^}" == "xBROADCOM" ]; then
	...
	export CXX=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-g++
	export AR=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-ar
	export LD=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-ld
	export NM=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-nm
	export RANLIB=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-ranlib
	export STRIP=${RDK_TOOLCHAIN_PATH}/bin/i686-cm-linux-strip

	export PATH=${PATH}:${RDK_TOOLCHAIN_PATH}
fi
...
function configure()
{
    ...
    configure_options="$configure_options --enable-testapp "
